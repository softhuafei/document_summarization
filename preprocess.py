...
"""
python -m pdb 
-train_src ./data/NLPCC/train.src 
-train_tgt ./data/NLPCC/train.tgt 
-valid_src ./data/NLPCC/valid.src 
-valid_tgt ./data/NLPCC/valid.tgt 
-save_data ./data//NLPCC/nlpcc
-src_vocab_size 4000 
-tgt_vocab_size 4000
-trun_src 2000 
-trun_tgt 57 
...
# -src_length 和 -trun_src使用一个就好
# -src_length，超过这个长度的句子会被过滤掉
# -trun_src,单独使用时，超过这个长度的部分会被截断
...
def main():
    ...
    train = makeData(opt.train_src, opt.train_tgt, dicts['src'], dicts['tgt'], sort=False)

    print('Preparing validation ...')
    valid = makeData(opt.valid_src, opt.valid_tgt, dicts['src'], dicts['tgt'], sort=False)
